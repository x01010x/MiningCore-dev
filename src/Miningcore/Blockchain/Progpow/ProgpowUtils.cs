...
public static class ProgpowUtils
{
    ...
    public static string EvrmoreEncodeTarget(double difficulty)
    {
        difficulty = 1.0 / difficulty;
        ...
        NewTarget = BigInteger.Multiply(EvrmoreConstants.Diff1B, new BigInteger(difficulty));
        ...
        if(DecimalOffset > -1)
        {
            int Precision = (StringDiff.Length - 1) - DecimalOffset;
            DecimalDiff = BigInteger.Parse(StringDiff.Substring(DecimalOffset + 1));
            DecimalTarget = BigInteger.Multiply(EvrmoreConstants.Diff1B, DecimalDiff);
